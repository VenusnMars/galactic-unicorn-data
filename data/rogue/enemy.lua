...
function Enemy.mt:decide()
  ...
  local distance_to_player = Vector2.manhattan_distance(player.get_position(), self.pos)
  ...
  elseif distance_to_player <= DETECTION_DISTANCE then
    ...
    self.path = a_star(self.pos, player.get_position(), can_move)
    if self.path then
