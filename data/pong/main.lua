...
local BALL_SPEED_0  = 13.0
local BALL_SPEED_MAX = 25.0

ACCELERATION_RATIO = 1.08
...
function update(dt)
  ...
  if ball_vel_x < 0 and ball_x < LEFT_PADDLE_FACE_X then
    if ball_y >= left_paddle_y - 0.5 and ball_y <= left_paddle_y + PADDLE_HEIGHT + 0.5 then
      ...
      local bounced_speed = math.min(math.abs(ball_vel_x) * ACCELERATION_RATIO, BALL_SPEED_MAX)
